...
using SavedSelection = winrt::UsefulAIKey::SavedSelection;
using CustomApp = winrt::UsefulAIKey::CustomApp;
...
IAsyncOperation<SavingError> DataStorage::RemoveCustomAppAsync(hstring path)
{
	JsonObject root{ nullptr };
	...
	JsonArray customApps;
	if (root.HasKey(L"customApps")) customApps = root.GetNamedArray(L"customApps");
	else co_return SavingError::None;

	for (uint32_t i = 0; i < customApps.Size(); i++)
	{
		if (customApps.GetAt(i).GetObject().GetNamedString(L"path", L"") == path)
		{
